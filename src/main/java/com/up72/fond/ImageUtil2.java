...
public class ImageUtil2 {
    ...
    public static String zoom(String sourcePath , String targetPath, int width , int height) throws IOException{
        ...
        if(IMAGE_FORMAT.GIF.getValue() == formatName){
            ...
            encoder.setRepeat(0);
            for (int i = 0; i < decoder.getFrameCount(); i ++) {
//                BufferedImage image =Thumbnails.of(decoder.getFrame(i)).size(500, 500).asBufferedImage(); // 等比缩小
                BufferedImage image = zoom2(decoder.getFrame(i), width, height);
                encoder.setDelay(decoder.getDelay(i));
                encoder.addFrame(image);
            }
            encoder.finish();
     ...
     * @param height          压缩图片宽度
     */
    private static BufferedImage zoom2(BufferedImage sourceImage , int width , int height){
        BufferedImage tag = new BufferedImage(width, height, BufferedImage.TYPE_INT_RGB);
        tag.getGraphics().setColor(Color.WHITE);
        tag.getGraphics().fillRect(0, 0, width, height);
        tag.getGraphics().drawImage(
                sourceImage.getScaledInstance(width, height, Image.SCALE_SMOOTH), 2, 2,
                null);
        return tag;
    }
...
//        gc.setColor(Color.WHITE);
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            File file = new File("E:\\testImageDeal/ec84.gif");
            BufferedImage image = ImageIO.read(file);
...
//            ImageUtil2.zoom("E:\\testImageDeal/ec84.gif", bigTargetPath, 100, 100);

            String smallTargetPath = pathPrefix  + System.currentTimeMillis() + pathSuffix;
//            ImageUtil2.zoom("E:\\testImageDeal/ec84.gif", "E:\\testImageDeal/"+System.currentTimeMillis()+".gif", 300,300);
